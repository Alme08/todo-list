...
window.addEventListener("click", (e) => {
  ...
  switch (e.target.id) {
    ...
    case "editTodo":
      ...
      editDescriptionT.value =
        getTodo(indexProject)[indexTodo].getDescription();
      editDueDateT.value = getTodo(indexProject)[indexTodo].getDueDate();
      for (let i = 0; i < editPriorityT.length; i++) {
        if (
          editPriorityT[i].value ==
          getTodo(indexProject)[indexTodo].getPriority()
        ) {
    ...
    case "todoLi":
      ...
      todoDescriptionInfo.textContent =
        getTodo(indexProject)[indexTodo].getDescription();
      todoDueDateInfo.textContent =
        getTodo(indexProject)[indexTodo].getDueDate();
...
const setLocalStorage = () => {
  ...
  for (let i = 0; i < currentProjects.length; i++) {
    ...
    for (let j = 0; j < currentProjects[i]._todoItems.length; j++) {
      currentProjects[i]._todoItems[j] = new TodoItem(
        currentProjects[i]._todoItems[j].title,
        currentProjects[i]._todoItems[j].description,
        currentProjects[i]._todoItems[j].dueDate,
        currentProjects[i]._todoItems[j].priority,
        currentProjects[i]._todoItems[j].indexProject
      );
    }
  }
  for (let i = 0; i < currentProjects.length; i++) {
